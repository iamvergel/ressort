...
$stmt = $pdo->prepare("SELECT * FROM inquiries WHERE room = 'Amacan' AND room = 'pending'"); //AND room = 'pending'
$stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['id'])) {
    ...
    $updateStmt = $pdo->prepare("UPDATE inquiries SET status = 'accepted' WHERE id = :id");
    ...
    // Redirect back
    header('Location: /adminInquiries/Amacan');
    exit();
}
?>
...
<body>
    ...
    <div class="content" style="overflow-y: scroll">
        ...
        <div class="row w-100 my-5">
            <button class="col btn btn-warning p-3 mx-2 border-0 rounded-2 shadow-lg text-light fs-2 fw-bold amacan"><i
                    class="bi bi-house-fill me-2"></i>Amacan</button>
            <button class="col btn btn-success p-3 mx-2 border-0 rounded-2 shadow-lg text-light fs-2 fw-bold vrhouse"><i
                    class="bi bi-house-door-fill me-2"></i>VR House</button>
            <button class="col btn btn-primary p-3 mx-2 border-0 rounded-2 shadow-lg text-light fs-2 fw-bold 22hrs"><i
                    class="bi bi-clock-fill me-2"></i>22 Hours</button>
        </div>
                            ...
                            <td>
                                <form action="/adminInquiries/Amacan" method="POST" style="display:inline;">
                                    ...
                                    <button class="btn btn-success btn-sm" style="font-size: 12px" type="submit">
                                        <i class="bi bi-check-circle me-1 d-none d-xl-block"></i>Accept
                                    </button>
                                </form>
                                <form action="decline_inquiry.php" method="POST" style="display:inline;">
                                    <input type="hidden" name="id" value="<?php echo htmlspecialchars($inquiry['id']); ?>">
                                    <button class="btn btn-danger btn-sm" style="font-size: 12px" type="submit">
                                        <i class="bi bi-x-circle me-1 d-none d-xl-block"></i>Decline
                                    </button>
                                </form>
                            </td>
                        </tr>
